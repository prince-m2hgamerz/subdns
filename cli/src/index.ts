...
function exitWithError(msg: string, suggestion?: string): never {
...
function findSubdomain(name: string, subdomains: Record<string, unknown>[]): Record<string, unknown> | undefined {
  return subdomains.find(
    (s) => (s.name as string) === name || (s.fullDomain as string) === name
  );
}
...
Example:
  ...
  .action(async () => {
    ...
    if (apiKey) {
      ...
      try {
        const data = await get<{ subdomains: unknown[] }>("/api/subdomains", 5000);
...
Examples:
  ...
  .action(async () => {
    ...
    try {
      ...
      const hdr = (s: string, w: number) => chalk.bold(s.padEnd(w));
      ...
      for (const r of rows) {
        ...
        console.log(
          `  ${statusColor(r.domain.padEnd(c1))}` +
...
Examples:
  ...
  .action(async (name: string) => {
    requireAuth();
    try {
      const all = await get<{ subdomains: Record<string, unknown>[] }>("/api/subdomains");
      const sd = findSubdomain(name, all.subdomains);
      if (!sd) {
        exitWithError(`Subdomain "${name}" not found`, "subdns list");
      }
      const data = await get<{ subdomain: Record<string, unknown> }>(`/api/subdomains/${sd.id}`);
      const sub = data.subdomain;

      console.log();
      console.log(`  ${chalk.bold(sub.fullDomain as string)}`);
      console.log(`  ${chalk.dim("─".repeat(Math.min(String(sub.fullDomain ?? "").length, 48)))}`);
      for (const [key, value] of Object.entries(sub)) {
        if (key === "fullDomain" || key === "id") continue;
        const label = key.replace(/([A-Z])/g, " $1").replace(/^./, (s) => s.toUpperCase());
        const display = value == null ? chalk.dim("-") : String(value);
        console.log(`  ${label}:${" ".repeat(Math.max(1, 18 - label.length))} ${display}`);
      }

      const records = sub.dnsRecords as Record<string, unknown>[] | undefined;
      if (records && records.length > 0) {
        console.log();
        console.log(`  DNS Records:`);
        const mc = Math.max(...records.map((r) => (r.type as string).length), 4) + 2;
        for (const rec of records) {
          const ttl = rec.ttl != null && rec.ttl !== 1 ? `TTL: ${rec.ttl}` : "TTL: auto";
          console.log(`    ${(rec.type as string).padEnd(mc)}${rec.content}  (${ttl})`);
        }
      }
      console.log();
    } catch (err) {
  ...
  .action(async (opts: { limit: string }) => {
    ...
    try {
      const data = await get<{ activities: Record<string, unknown>[] }>(
        `/api/activity?limit=${Math.min(Math.max(1, parseInt(opts.limit) || 20), 100)}`
      );
      ...
      const rows = data.activities.map((a) => ({
        time: formatTime((a.createdAt ?? a.created_at) as string),
        event: String(a.event ?? a.type ?? "-"),
        description: String(
          typeof a.description === "string"
            ? (() => { try { const p = JSON.parse(a.description); return p.description ?? p.name ?? a.description; } catch { return a.description; } })()
            : a.description ?? ""
        ),
      }));
